...
export default function PlanlamaPage() {
  ...
  async function load() {
    const [{ data: projeler }, { data: plani }, { data: mak }] = await Promise.all([
      ...
      supabase.from('uretim_plani').select('*, proje:proje(proje_no,ad,cikti_turu,renk_sayisi,kazan_sayisi,baskili_yuz,is_tipi,musteri:musteri_tanim(ad)), makine:makine_tanim(ad,tur,fason)').order('planlanan_tarih', { ascending: true, nullsFirst: false }),
                        ...
                        <div key={p.id} className="flex items-center justify-between text-xs bg-gray-50 rounded-lg px-2 py-1.5 gap-2">
                          <div className="min-w-0">
                            <div className="truncate">
                              ...
                              {m.tur === 'baski' && p.proje?.is_tipi && p.proje.is_tipi !== 'yeni' && (
                                <span className={`badge ml-1 !px-1.5 !py-0 !text-[10px] ${p.proje.is_tipi === 'revizyon' ? 'badge-red' : 'badge-amber'}`}>
                                  {p.proje.is_tipi === 'revizyon' ? 'Revizyon' : 'Tekrar'}
                                </span>
                              )}
                            </div>
                            {m.tur === 'baski' && (
                              <div className="truncate text-gray-500">
                                {p.proje?.musteri?.ad || '—'} · {p.proje?.ad}
                                {p.proje?.renk_sayisi ? ` · ${p.proje.renk_sayisi} renk` : ''}
                                {p.proje?.kazan_sayisi ? ` · ${p.proje.kazan_sayisi} kazan` : ''}
                                {p.proje?.baskili_yuz ? ` · ${p.proje.baskili_yuz === 'alt' ? 'alt baski' : 'ust baski'}` : ''}
                              </div>
                            )}
          ...
          return (
            <div key={projeId} className="card p-0 overflow-hidden">
              ...
              <table className="table-base">
                <thead>
                  <tr><th>#</th><th>Adim</th><th>Makine</th><th>Tarih</th><th>Saat</th><th>Sure (dk)</th><th>Hedef kg</th><th>Hedef m</th><th>Hammadde</th><th>Durum</th></tr>
                </thead>
                <tbody>
                  {adimlar.sort((a, b) => a.adim_sira - b.adim_sira).map(a => (
                    <tr key={a.id}>
                      ...
                      <td><input type="number" className="!w-20 !py-1 !text-xs" placeholder="—" value={a.hedef_kg ?? ''} onChange={e => alanGuncelle(a.id, { hedef_kg: e.target.value ? parseFloat(e.target.value) : null })} /></td>
                      <td><input type="number" className="!w-20 !py-1 !text-xs" placeholder="—" value={a.hedef_metre ?? ''} onChange={e => alanGuncelle(a.id, { hedef_metre: e.target.value ? parseFloat(e.target.value) : null })} /></td>
                      <td>
