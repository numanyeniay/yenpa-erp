...
export default function YeniProjePage() {
  ...
  const katoEni = katoEniHesapla({
    ...form,
    en_mm: parseFloat(form.en_mm) || 0,
    boy_mm: parseFloat(form.boy_mm) || 0,
    kurek_mm: parseFloat(form.kurek_mm) || 0,
    kapak_mm: parseFloat(form.kapak_mm) || 0,
    bobin_en_mm: parseFloat(form.bobin_en_mm) || 0,
    yan_yana_baski: parseInt(form.yan_yana_baski) || 1,
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    // Proje no oluştur
    const { data: sayac } = await supabase.from('proje').select('id', { count:'exact', head:true })
    const no = `YEN-${new Date().getFullYear()}-${String((sayac || 0) + 1).padStart(4,'0')}`
    ...
    const { data: proje, error } = await supabase.from('proje').insert({
      ...
      en_mm: parseFloat(form.en_mm) || null,
      boy_mm: parseFloat(form.boy_mm) || null,
      kurek_mm: parseFloat(form.kurek_mm) || null,
      kapak_mm: parseFloat(form.kapak_mm) || null,
      bobin_en_mm: parseFloat(form.bobin_en_mm) || null,
      bobin_cap_mm: parseFloat(form.bobin_cap_mm) || null,
      bobin_metre: parseFloat(form.bobin_metre) || null,
      baskili: form.baskili,
      baskili_yuz: form.baskili ? form.baskili_yuz : null,
      renk_sayisi: form.baskili ? parseInt(form.renk_sayisi) : null,
      kato_eni_mm: katoEni,
      yan_yana_baski: parseInt(form.yan_yana_baski),
      zip_var: form.zip_var,
      sonic_var: form.sonic_var,
      mexika_deligi: form.mexika_deligi,
      kargo_bandi: form.kargo_bandi,
      kenar_tirasi_mm: parseFloat(form.kenar_tirasi_mm) || null,
      notlar: form.notlar,
      durum: 'taslak',
    }).select().single()
  ...
  const cikti = form.cikti_turu
  const showBobin = cikti === 'bobin'
  const showEbat = cikti !== 'bobin'
      ...
      <form onSubmit={handleSave} className="space-y-4">
        ...
        <div className="card">
          ...
          <div className="card-body space-y-4">
            <div className="form-grid-2">
              ...
              <div><label>Urun / proje adi</label>
                <input value={form.ad} onChange={e => setF('ad', e.target.value)} placeholder="Ornek: Silka Okul Seti Torbasi" required />
              </div>
            </div>
            <div><label>Aciklama (opsiyonel)</label>
              <textarea value={form.aciklama} onChange={e => setF('aciklama', e.target.value)} rows={2} placeholder="Musteri notu veya ozel istek..." />
            </div>
          </div>
        </div>

        {/* Cikti turu ve ebatlar */}
        <div className="card">
          <div className="card-header"><span className="font-medium">Cikti turu ve ebatlar</span></div>
          <div className="card-body space-y-4">
            <div><label>Cikti turu</label>
            ...
            {showBobin && (
              <div className="form-grid-3">
                <div><label>Bobin eni (mm)</label><input type="number" value={form.bobin_en_mm} onChange={e => setF('bobin_en_mm', e.target.value)} placeholder="350" /></div>
                <div><label>Bobin capi (mm)</label><input type="number" value={form.bobin_cap_mm} onChange={e => setF('bobin_cap_mm', e.target.value)} placeholder="300" /></div>
                <div><label>Bobin metre</label><input type="number" value={form.bobin_metre} onChange={e => setF('bobin_metre', e.target.value)} placeholder="1500" /></div>
              </div>
            )}

            {showEbat && (
              <div className="form-grid-3">
                <div><label>En (mm)</label><input type="number" value={form.en_mm} onChange={e => setF('en_mm', e.target.value)} placeholder="200" /></div>
                <div><label>Boy (mm)</label><input type="number" value={form.boy_mm} onChange={e => setF('boy_mm', e.target.value)} placeholder="300" /></div>
                <div><label>Kurek / korpus (mm)</label><input type="number" value={form.kurek_mm} onChange={e => setF('kurek_mm', e.target.value)} placeholder="50" /></div>
                <div><label>Kapak payi (mm)</label><input type="number" value={form.kapak_mm} onChange={e => setF('kapak_mm', e.target.value)} placeholder="50" /></div>
                ...
                <div><label>Kenar tirasi (her taraftan mm)</label><input type="number" value={form.kenar_tirasi_mm} onChange={e => setF('kenar_tirasi_mm', e.target.value)} placeholder="5" step="0.5" /></div>
              </div>
            )}

            {katoEni > 0 && (
              <div className="bg-blue-50 rounded-lg px-4 py-3 text-sm text-blue-800">
                Hesaplanan kato eni (makineye girecek ham bobin eni): <strong>{katoEni.toFixed(0)} mm</strong>
                {parseFloat(form.yan_yana_baski) > 1 && <span className="ml-2 text-blue-600">({form.yan_yana_baski} adet yan yana)</span>}
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {[
                  { key:'zip_var',       label:'Zip / fermuar' },
                  { key:'sonic_var',     label:'Sonic islem' },
                  { key:'mexika_deligi', label:'Meksika sapkasi deligi' },
                  { key:'kargo_bandi',   label:'Kargo bandi' },
                ].map(opt => (
        ...
        <div className="card">
          <div className="card-header">
            ...
            <button type="button" onClick={katmanEkle} className="btn btn-sm btn-primary">
              + Katman ekle
            </button>
          </div>
            ...
            {katmanlar.map((k, i) => (
              <div key={i} className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {katmanlar.length > 1 && (
                    <button type="button" onClick={() => katmanSil(i)}
        ...
        {rota.length > 0 && (
          <div className="card">
            <div className="card-header"><span className="font-medium">Otomatik hesaplanan uretim rotasi</span></div>
              ...
              <div className="flex flex-wrap gap-2">
                {rota.map((adim, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    ...
                    {i < rota.length - 1 && <span className="text-gray-300 text-sm">→</span>}
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-400 mt-3">Bu rota katman yapisina gore otomatik olusturulmustur. Planlama adiminda makine ve tarih atanacak.</p>
            </div>
          </div>
        )}
        ...
        <div className="card">
          <div className="card-body">
            ...
            <textarea value={form.notlar} onChange={e => setF('notlar', e.target.value)} rows={3}
              placeholder="Ozel uretim notu, musteri talebi..." />
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving} className="btn btn-primary">
            {saving ? 'Kaydediliyor...' : 'Projeyi kaydet ve fiyatlamaya gec'}
          </button>
